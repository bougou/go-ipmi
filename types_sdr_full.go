...
func parseSDRFullSensor(data []byte, sdr *SDR) error {
	...
	if len(data) < minSize {
		return fmt.Errorf("sdr (full sensor) data length (%d) must be >= %d", len(data), minSize)
	}
	...
	if len(data) < minSize+idStrLen {
		return fmt.Errorf("sdr (full sensor) data length (%d) must be >= %d", len(data), minSize+idStrLen)
	}
