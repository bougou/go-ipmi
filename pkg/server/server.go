...
func (s *Server) handleRMCPPlus(addr net.Addr, pkt []byte) {
	sessionID, _, payloadType, flags, payload, ok := protocol.ParseRMCPPlusHeader(pkt)
	if !ok {
		return
	}
...
func (s *Server) sendRMCPPlusSession(addr net.Addr, payloadType, flags uint8, sess *bmc.Session, payload []byte) {
	pkt := protocol.BuildRMCPPlusPacket(payloadType, flags, sess.BMCID, sess.OutboundSeq, payload)
	_, _ = s.conn.WriteTo(pkt, addr)
}
...
func encryptPayload(plain, k2 []byte) ([]byte, error) {
	padded, padLen := aesPad(plain)
	iv := randomBytes(16)
	encrypted, err := encryptAES(padded, k2[:16], iv)
	if err != nil {
		return nil, err
	}
	out := make([]byte, 16+len(encrypted)+1)
	copy(out[:16], iv)
	copy(out[16:], encrypted)
	// Spec §13.29: confidence pad length byte follows the ciphertext.
	out[16+len(encrypted)] = byte(padLen)
	return out, nil
}
