...
func handleGetChassisCapabilities(_ context.Context, _ *HandlerContext, _ []byte) ([]byte, types.CompletionCode, error) {
	...
	return []byte{0x00, 0x20, 0x20, 0x20, 0x20}, types.CodeOK, nil
}
...
func handleGetChassisStatus(ctx context.Context, hctx *HandlerContext, _ []byte) ([]byte, types.CompletionCode, error) {
	...
	if ch != nil {
		...
		if err != nil {
			...
			// 80h (types.CodeParameterNotSupported) is only valid for
...
func handleSetSystemBootOptions(ctx context.Context, hctx *HandlerContext, req []byte) ([]byte, types.CompletionCode, error) {
	if len(req) < 1 {
		return nil, types.CodeRequestDataTruncated, nil
	}
...
//   - 0x05: Boot Flags
func handleGetSystemBootOptions(ctx context.Context, hctx *HandlerContext, req []byte) ([]byte, types.CompletionCode, error) {
	if len(req) < 1 {
		return nil, types.CodeRequestDataTruncated, nil
	}
	paramSelector := types.BootOptionParamSelector(req[0] & 0x7f)
