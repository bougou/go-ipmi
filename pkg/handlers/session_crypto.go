...
// hmacKey returns the HMAC key for RAKP2/3: KG if configured, otherwise Kuid
// (the user's password padded to 20 bytes).
func hmacKey(sess *bmc.Session, b *bmc.BMC) []byte {
	if len(b.KG) > 0 {
		return b.KG
	}
