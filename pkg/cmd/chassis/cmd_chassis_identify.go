...
func (req *ChassisIdentifyRequest) Pack() []byte {
	out := make([]byte, 2)
	ipmi.PackUint8(uint8(req.IdentifyInterval), out, 0)

	var force uint8 = 0
	if req.ForceIdentifyOn {
		force = 1
	}
	ipmi.PackUint8(force, out, 1)
	return out
}

func (req *ChassisIdentifyRequest) Command() ipmi.Command {
	return ipmi.CommandChassisIdentify
}
