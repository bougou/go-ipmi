...
func (req *GetSensorReadingFactorsRequest) Command() ipmi.Command {
	return ipmi.CommandGetSensorReadingFactors
}

func (req *GetSensorReadingFactorsRequest) Pack() []byte {
	out := make([]byte, 2)
	ipmi.PackUint8(req.SensorNumber, out, 0)
	ipmi.PackUint8(req.Reading, out, 1)
	return out
}
...
func (res *GetSensorReadingFactorsResponse) Unpack(msg []byte) error {
	...
	res.NextReading, _, _ = ipmi.UnpackUint8(msg, 0)

	b1, _, _ := ipmi.UnpackUint8(msg, 1)
	b2, _, _ := ipmi.UnpackUint8(msg, 2)

	m := uint16(b2&0xc0)<<2 | uint16(b1)
	res.M = int16(ipmi.TwoSComplement(uint32(m), 10))

	res.Tolerance = b2 & 0x3f

	b3, _, _ := ipmi.UnpackUint8(msg, 3)
	b4, _, _ := ipmi.UnpackUint8(msg, 4)
	b5, _, _ := ipmi.UnpackUint8(msg, 5)

	b := uint16(b4&0xc0)<<2 | uint16(b3)
	res.B = int16(ipmi.TwoSComplement(uint32(b), 10))

	res.Accuracy = uint16(b5&0xf0)<<2 | uint16(b4&0x3f)
	res.Accuracy_Exp = (b5 & 0x0c) >> 2

	b6, _, _ := ipmi.UnpackUint8(msg, 6)

	rExp := uint8((b6 & 0xf0) >> 4)
	res.R_Exp = int8(ipmi.TwoSComplement(uint32(rExp), 4))

	bExp := uint8(b6 & 0x0f)
	res.B_Exp = int8(ipmi.TwoSComplement(uint32(bExp), 4))
