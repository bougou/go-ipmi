...
IPMI specification commands are request/response pairs, exposed here as methods
on `client.Client`.

An `ipmitool` subcommand may map to one IPMI command or to a sequence of them
(`ipmitool sdr list` loops `GetSDR`). Library helpers that are not single wire
