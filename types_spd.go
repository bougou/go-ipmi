...
// You can inspect memory spd fields through tools like CPU-Z.
//
// 模组存在的串行检测，是通过 IIC 串行接口的 EEPROM 对内存插槽中的模组存在的信息检查。
// SPD 是一组关于内存模组的配置信息，如 P-Bank 数量、电压、行地址/列地址数量、位宽、各种主要操作时序（如CL、tRCD、tRP、tRAS等）。
// SPD 存放在一个容量为 256 字节的 EEPROM（Electrically Erasable Programmable Read Only Memory，电擦除可编程只读存储器）中。
// 实际上在 SPD 中，JEDEC 规定的标准信息只用了 128 个字节（还有 128 字节，属于厂商自己的专用区）。
// 同主板的 BIOS 一样，SPD 也是可以刷新的。用 Thaiphoon Burner 这款软件就能做到。
//
// The SPD (Serial Presence Detect) chip on a memory module is an additional chip
// holding 128Hex bytes of information about the module.
// This identifies the module to the BIOS during POST so the Motherboard knows
// its characteristics and timings that can be used.
// This was introduced at the same time as SDRAM.
//
// For a computer system to recognize a memory module, the module needs to have a chip on it
// that communicates the specifications of the module to the system.
// A system's basic input/output system (BIOS) may depend on serial presence detect (SPD) data
// to properly configure and optimize the memory sub system.
//
// SPD information is written to a single EEPROM that resides on the memory module (DIMM).
// The pins of the EEPROM are routed to the edge connector of the module so that it may be
// accessed by the system through the I2C or SMBus.
// The Serial Presence Detect (SPD) standard is intended for use on any memory module,
// independent of memory technology or module form factor.
//
