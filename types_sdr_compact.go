...
func parseSDRCompactSensor(data []byte, sdr *SDR) error {
	...
	if len(data) < minSize {
		return fmt.Errorf("sdr (compact sensor) data length (%d) must be >= %d", len(data), minSize)
	}
	...
	if len(data) < minSize+idStrLen {
		return fmt.Errorf("sdr (compact sensor) data length (%d) must be >= %d", len(data), minSize+idStrLen)
	}
