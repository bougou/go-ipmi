...
func NewCmdChannelInfo() *cobra.Command {
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			res2, err := client.GetChannelAccess(ctx, channelNumber, types.ChannelAccessOption_Volatile)
			if err != nil {
			...
			res3, err := client.GetChannelAccess(ctx, channelNumber, types.ChannelAccessOption_NonVolatile)
			if err != nil {
				CheckErr(fmt.Errorf("GetChannelAccess failed, err: %w", err))
			}
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			for _, record := range cipherSuiteRecords {
				fmt.Printf("%-5d%-8d%-16s%-16s%-s\n", record.CipherSuitID, record.OEMIanaID, types.AuthAlg(record.AuthAlg), types.IntegrityAlg(record.IntegrityAlgs[0]), types.CryptAlg(record.CryptAlgs[0]))
			}
		},
	}
	return cmd
